...
def test_work_tasks_ui_is_applied_after_protected_frontend_patch():
    ...
    execution_patch = Path("scripts/patch-work-execution-ui.py").read_text(encoding="utf-8")
    ...
    assert "python scripts/patch-work-execution-ui.py" in build_script
    ...
    assert build_script.index("patch-work-tasks-ui.py") < build_script.index("patch-work-execution-ui.py")
    assert build_script.index("patch-work-execution-ui.py") < build_script.index("patch-work-preview-bridge.py")
    ...
    assert "AssistantAgentExecution" in execution_patch
    assert "Execução por agente" in execution_patch
    assert "task.executions" in execution_patch
    ...
    assert 'loadAssistantCloudFiles(accessToken)' in cloud_patch
...
def test_work_panel_does_not_modify_protected_public_auth_markers():
    patches = [
        ...
        Path("scripts/patch-work-execution-ui.py").read_text(encoding="utf-8"),
        Path("scripts/patch-work-preview-bridge.py").read_text(encoding="utf-8"),
        Path("scripts/patch-work-cloud-sync.py").read_text(encoding="utf-8"),
    ]
